...
class NotificationService {
  ...
  /// 立刻发一条测试通知（v1.7.6 加的诊断工具）。
  ///
  /// 为什么要有这个：
  /// 「到点不提醒」这类问题最难的地方是**没法当场验证** —— 排好的提醒最快也要
  /// 等几分钟、最慢得等到第二天早上那节课。以前只能干等，排查一轮要一天。
  ///
  /// 有了这个按钮，点一下立刻就能确认「通知这条链路到底通不通」：
  ///   · 通知栏弹出来了 → 权限、图标、渠道都没问题，剩下的只是排程的时机问题
  ///   · 点了没反应 → 权限被系统掐了（或国产 ROM 拦了），跟排程无关
  ///   · 点了直接闪退 → 通知图标/原生层有问题（v1.7.6 之前就是这个情况）
  ///
  /// 注意：它走的是和真实提醒**完全相同**的通知渠道和小图标，
  /// 所以它能代表真实提醒的表现。
  static Future<void> showTestNotification() async {
    await init();
    await _plugin.show(
      // 用一个固定 id，重复点就是覆盖同一条，不会刷出一串
      _idOf('__test_notification__'),
      '这是一条测试通知',
      '如果你看到这条，说明通知能正常弹出来了 ✓',
      const NotificationDetails(android: _noteDetails),
    );
  }
}
